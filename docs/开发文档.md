# comfyui_workflow_preset_switch 开发文档
...
- 记录与应用节点 bypass 状态。
- 支持通过 `WorkflowPresetSwitch` 节点输入 `preset_index: INT` 触发切换。
- 支持多个 preset（动态存储，不限制为 10）。

### Phase 2（扩展）

- 记录/恢复节点 Widget 参数（步数、CFG、Sampler 设置等）。
- 提供参数白名单/黑名单机制。
...
1. **前端（web/workflow_preset_switch.js）**
   - 读取当前画布节点状态。
   - 记录 preset。
   - 应用 preset。
...
2. **后端（nodes.py）**
   - 定义 `WorkflowPresetSwitch` 节点。
   - 提供 `preset_index` 输入，作为流程中的显式切换控制入口。

3. **持久化策略**
   - Phase 1 优先存到 workflow metadata（随工作流保存）。
...
├─ __init__.py                    # 节点映射导出
├─ nodes.py                       # WorkflowPresetSwitch 节点
├─ web/
│  ├─ workflow_preset_switch.js   # 前端核心逻辑（记录/应用/监听）
│  └─ style.css                   # 样式（可选）
├─ docs/
│  ├─ 开发文档.md
│  └─ 需求简述.ini
...
- M2：Phase 1 记录/应用 bypass preset
- M3：index 自动触发切换
- M4：异常处理与文档完善
- M5：Phase 2/3 预留接口
